...
leodido.controller.Caret = function($scope, $element, $attrs) {
  ...
  var getIndex = function(pos) {
    // TODO: replace space with non breaking spaces (/\s/g, '\u00a0') ?
    var value = getValue().replace(/\r/g, '') ,// TODO: use (/(?:\r\n|\r)+/g, \n) ?
        len = value.length,
        position = ~~pos;
    // Enforce boundaries
  ...
  /**
   * Retrive the start and end positions
   *
   * @return {!leodido.typedef.Selection}
   */
  ctrl.getSelection = getSelectionPolyfill;

  goog.exportProperty(ctrl, 'getPosition', ctrl.getPosition);
  goog.exportProperty(ctrl, 'getSelection', ctrl.getSelection);
  goog.exportProperty(ctrl, 'setPosition', ctrl.setPosition);
  goog.exportProperty(ctrl, 'getNamespace', ctrl.getNamespace);
...
// FIXME: replace CR in returned text?
