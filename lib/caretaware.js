...
leodido.directive.CaretAware.prototype.controller = function($scope, $element, $attrs) {
  ...
  var getIndex = function(pos) {
    ...
    leodido.constants.DEBUG && console.log('?', len, pos, position);
  ...
  $scope.$watch(
      ...
      function(val, oldval) {
        leodido.constants.DEBUG && console.log('WATCH', ctrl.getPosition(), val, oldval, getIndex(val));
        if (ctrl.getPosition() !== val) {
          leodido.constants.DEBUG && console.log('SET', val);
...
leodido.directive.CaretAware.prototype.link = function($scope, $elem, $attrs, $ctrl) {
  ...
  $elem.on('keydown keyup click', function() {
    ...
    $scope.$evalAsync(function() {
      ...
      leodido.constants.DEBUG && console.log('EVAL ASYNC', $scope[$ctrl.getNamespace()]);
    });
  });
};
